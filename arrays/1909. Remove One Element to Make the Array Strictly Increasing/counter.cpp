...
to their left. When we increment the counter, it is equivalent to replacing the previous element. 
On line 16, we replace the current element with its previous element if it is still smaller than the 
element two spaces before it. This is equivalent to removing the current element, and ensuring that 
